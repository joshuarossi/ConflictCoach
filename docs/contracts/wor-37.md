...
**Updated InviteSharingViewProps note:** `otherPartyName` comes from Router state (set by NewCasePage from the form value), not from `api.cases.get`. The `api.cases.get` query's `otherPartyName` resolves the *other user's* display name, which is empty until the invite is redeemed.
