...
describe("WOR-57: Logged-out invite view", () => {
  test("AC1: displays initiator name in invitation message", () => {
    ...
    expect(
      screen.getByText(
        /Alex.*has invited you to work through something together/i,
      ),
    ).toBeInTheDocument();
  });
  ...
  test("AC1: shows product explanation text", () => {
    ...
    expect(screen.getByText(/conflict coach/i)).toBeInTheDocument();
  });
  ...
  test("AC1: does NOT display private data (description, desiredOutcome)", () => {
    ...
    expect(
      screen.queryByText(/Alex feels undervalued in team meetings/i),
    ).not.toBeInTheDocument();
    expect(
      screen.queryByText(/Better communication and mutual respect/i),
    ).not.toBeInTheDocument();
  });
});
