...
describe("AC: Typography scale matches StyleGuide §3.3", () => {
  ...
  const EXPECTED_SIZES: Record<string, { size: number; lineHeight: number | string }> = {
  ...
  test("tailwind config defines fontSize entries for the full typography scale", async () => {
    ...
    for (const role of Object.keys(EXPECTED_SIZES)) {
      expect(
        fontSize,
        `Missing fontSize entry for "${role}"`,
      ).toHaveProperty(role);
    }
  });
  ...
  test("heading weights are 500 (not 700) — verified in globals.css or theme.ts", () => {
    ...
    const heading700 = /h[1-3].*font-weight\s*:\s*700|heading.*700|700.*heading/i;
    expect(
      combined,
      "Headings must NOT use font-weight 700",
    ).not.toMatch(heading700);
  });
});
