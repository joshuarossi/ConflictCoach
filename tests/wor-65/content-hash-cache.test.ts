...
const CANNED_SUMMARY_A =
  "Summary A: discussed frustrations about communication.";
...
describe("Summary is cached by content hash for reuse", () => {
  ...
  test("calling compressMessages twice with the same messages only calls Haiku once", async () => {
    ...
    const result1 = await compressMessages(
      messagesA,
      mockClient as unknown as import("@anthropic-ai/sdk").default,
    );
    const result2 = await compressMessages(
      messagesA,
      mockClient as unknown as import("@anthropic-ai/sdk").default,
    );
  ...
  test("calling compressMessages with different messages calls Haiku again", async () => {
    ...
    await compressMessages(
      messagesA,
      mockClientA as unknown as import("@anthropic-ai/sdk").default,
    );
    await compressMessages(
      messagesB,
      mockClientA as unknown as import("@anthropic-ai/sdk").default,
