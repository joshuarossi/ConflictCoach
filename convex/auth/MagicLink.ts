  ...
  async sendVerificationRequest({
    ...
    const res = await fetch("https://api.resend.com/emails", {
      ...
      body: JSON.stringify({
        ...
        // Trust boundary: `url` is generated by the Convex Auth framework's
        // verification flow and is not user-controlled. We escape it defensively
        // to guard against future changes that might introduce user-controlled data.
        html: `<p>Click <a href="${url.replace(/&/g, "&amp;").replace(/"/g, "&quot;").replace(/</g, "&lt;").replace(/>/g, "&gt;")}">here</a> to sign in to Conflict Coach.</p>`,
      }),
    });
    if (!res.ok) {
